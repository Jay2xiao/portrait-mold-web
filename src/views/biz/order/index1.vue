...
function hasExternalRoute(row: any) {
  return ['PARTIAL_EXTERNAL', 'ALL_EXTERNAL'].includes(row?.routeSummaryMode || '');
}
  ...
  },
  {
    ...
    render(row: any) {
      return h(
        NTag,
        {
          type: routeSummaryTagType(row.routeSummaryMode),
          size: 'small'
        },
        {
          default: () => routeSummaryLabel(row.routeSummaryMode)
        }
      );
    }
  },
